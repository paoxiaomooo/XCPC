...
#define rs(u) (tr[u].rs)
    ...
    void pushdown(int u) {
        if (tr[u].tag) {
            swap(ls(u), rs(u));
            if (ls(u)) tr[ls(u)].tag ^= 1;
            if (rs(u)) tr[rs(u)].tag ^= 1;
            tr[u].tag = 0;
        }
    }
    ...
    void split_rank(int u, int k, int& x, int& y) {
        ...
        if (s == k) {
            ...
            x = u;
            split_rank(rs(u), k - s, rs(u), y);
        }
        pushup(u);
    }

    //维护序列时的插入,选择插入到哪个位置
    void insert_rank(T val, int k) {
        split_rank(root, k, T1, T2);
    ...
    //删除指定位置元素
    void erase_rank(int k) {
        split_rank(root, k - 1, T1, T2);
...
void NTT(vector<ll>& a, int n, int op) {//op=1为正变换,-1为逆变换
    a.resize(n);
    //reverse
    ...
    for (int m = 2;m <= n;m <<= 1) {
        ll g1 = qp(op == 1 ? G : inv_G, (P - 1) / m, P);
        for (int i = 0;i < n;i += m) {
            ll gk = 1;
            for (int j = 0;j < m / 2;j++) {
                ll x = a[i + j], y = a[i + j + m / 2] * gk % P;
                a[i + j] = (x + y) % P;a[i + j + m / 2] = (x - y + P) % P;gk = gk * g1 % P;
            }
        }
    }
    int inv_n = qp(n, P - 2, P);
    if (op == -1) for (int i = 0;i < n;i++) a[i] = a[i] * inv_n % P;
}
struct bigint {
    vector<char> nums;
    bigint(int x = 0) {
    ...
    friend ostream& operator<<(ostream& cout, bigint num) {
        bool start = false;
        for (int i = num.nums.size() - 1; i >= 0; i--) {
            if (!start && num.nums[i] == 0)
                continue;
            start = true;
            cout << char(num.nums[i] + '0');
        }
        if (!start)
            cout << 0;
        return cout;
...
bigint operator+(bigint a, bigint b) {
    ...
    res.clear();
    int t = 0;
    int mx = max(a.nums.size(), b.nums.size());
...
bigint operator-(bigint a, bigint b) {
    ...
    bigint sub(b);
    int flag = 0;
    int len = res.nums.size();
    while (sub.nums.size() < res.nums.size())
        sub.nums.push_back(0);
    for (int i = 0; i < len; i++) {
        if (res.nums[i] + flag >= sub.nums[i]) {
            res.nums[i] = res.nums[i] + flag - sub.nums[i];
            flag = 0;
        }
        else {
            res.nums[i] = res.nums[i] + 10 + flag - sub.nums[i];
            flag = -1;
        }
    }
    res.trim();
...
//     bigint res;
//     res.nums.resize(a.nums.size() + b.nums.size(), 0);
...
bigint operator*(bigint a, bigint b) {//nlogn
    ...
    int n = a.nums.size() - 1, m = b.nums.size() - 1;

    int lim = init_NTT(n + m);
    vector<ll> A(lim); for (int i = 0;i <= n;i++) A[i] = a.nums[i];
    vector<ll> B(lim);for (int i = 0;i <= m;i++) B[i] = b.nums[i];
...
bigint operator/(bigint a, bigint b) {
    ...
    while (n--) {
        ...
        while (!(tnum < b)) {
            int n1 = tnum.nums.size();
            for (int j = 0; j < n2; j++) {
                tnum.nums[j] -= b.nums[j];
                if (tnum.nums[j] < 0) {
                    tnum.nums[j + 1]--;
                    tnum.nums[j] += 10;
                }
